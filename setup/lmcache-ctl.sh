...
HTTP_PORT=${LMCACHE_HTTP_PORT:-8080}  # mgmt/metrics HTTP port; if taken by a foreign process
                                      # the whole server dies seconds after "up" (bind Errno 98
                                      # kills it AFTER the ZMQ port already passed the check)
