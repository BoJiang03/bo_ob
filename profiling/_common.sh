...
#
# COPY of setup/_common.sh for Part 3 (profiling), kept separate so the parts
# never interfere. Two differences from the setup copy:
#   * LMCACHE_PORT default is 5559 (profiling port block: 5559 / 8002 / 8082).
#   * model_start honours an optional NSYS_PREFIX launcher, so nsys.sh can start
#     vLLM under `nsys launch` (profiling armed but deferred) without changing the
#     rest of the engine. Empty NSYS_PREFIX == identical to the setup copy.
#
...
GPU=${GPU:-7}                       # global default GPU; override with GPU=0 / VLLM_GPU=0 on rtx-026
LMCACHE_PORT=${LMCACHE_PORT:-5559}  # profiling port block (setup uses 5556, l2_support 5557)
...
model_start() {
    ...
    # Optional nsys launcher (nsys.sh sets NSYS_PREFIX to "nsys launch ..."); empty = normal launch.
    local launcher=()
    [[ -n ${NSYS_PREFIX:-} ]] && read -r -a launcher <<< "$NSYS_PREFIX"
    gpu_mem_warn 55000 "vLLM (mem-util $GPU_MEM_UTIL) may OOM on top of it"
    echo "$MODEL_NAME: starting vLLM on port $VLLM_PORT (GPU $GPU, mem-util $GPU_MEM_UTIL)${kv_args:+, bound to lmcache on port $LMCACHE_PORT}${launcher:+ [under: ${launcher[*]}]}..."
    # VLLM_SERVER_DEV_MODE=1 enables POST /reset_prefix_cache (needed for cache benchmarks)
    VLLM_SERVER_DEV_MODE=1 setsid nohup "${launcher[@]}" vllm serve "$MODEL_PATH" \
