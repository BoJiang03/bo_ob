...
#
# COPY of setup/bench.sh for Part 3 (profiling); only the port defaults differ
# (VLLM_PORT 8002, LMCACHE_HTTP_PORT 8082 — the profiling port block, via this
# directory's _common.sh). Used to drive realistic mixed traffic (GPU-pool hits +
# L1 loads) under a flamegraph / nsys capture, e.g. KV_VOLUME > GPU pool < L1.
#
...
#   KV_VOLUME=30 ./bench.sh                   # overflow a 20GB GPU pool -> L1 loads
...
LMCACHE_HTTP_PORT=${LMCACHE_HTTP_PORT:-8082}   # MP server's HTTP endpoint (tokens-per-GB autodetect)
